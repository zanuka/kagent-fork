...
import { getTools, testTool } from '../actions/tools';

interface ToolComponent extends Component<ToolConfig> {
  name: string;
  provider: string;
}
...
export default function ToolsPage() {
  ...
  const [testingTool, setTestingTool] = useState<ToolComponent | null>(null);
        ...
        serversResponse.data.forEach((server) => {
          ...
          if (server.discoveredTools && Array.isArray(server.discoveredTools)) {
            server.discoveredTools.forEach((tool) => {
              // Ensure the component and its provider exist before adding
              if (tool && tool.component && tool.component.provider) {
                toolsFromServers.push(tool.component);
              } else {
                console.warn(
                  'Skipping discovered tool due to missing component or provider:',
                  tool
                );
              }
            });
  ...
  const handleTestTool = async (tool: ToolComponent) => {
    setTestingTool(tool);
    setIsTestLoading(true);
    try {
      const result = await testTool(tool.provider, tool.name);
      setTestResults((prev) => ({
        ...prev,
        [tool.name]: result.success,
      }));
    } catch {
      setTestResults((prev) => ({
        ...prev,
    ...
    <div className="mt-12 mx-auto max-w-6xl px-6">
                                  ...
                                  <Tooltip>
                                    <TooltipTrigger asChild>
                                      <Button
                                        ...
                                        className="h-8 w-8"
                                        onClick={() =>
                                          handleTestTool(tool as ToolComponent)
                                        }
                                      >
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              Test Tool: {testingTool && getToolDisplayName(testingTool)}
            </DialogTitle>
            <DialogDescription>
              {testingTool && getToolDescription(testingTool)}
            </DialogDescription>
          </DialogHeader>

          <div className="py-4">
            <div className="text-sm mb-2">Tool ID:</div>
            <code className="text-xs bg-secondary/10 p-2 block rounded">
              {testingTool && getToolIdentifier(testingTool)}
            </code>

            {testingTool && testResults[testingTool.name] === true && (
              <div className="mt-4 p-3 rounded bg-green-50 text-green-700">
                <span>Tool test successful</span>
              </div>
            )}
            {testingTool && testResults[testingTool.name] === false && (
              <div className="mt-4 p-3 rounded bg-red-50 text-red-700">
                <span>Tool test failed</span>
              </div>
            )}
          </div>

          <DialogFooter>
            <Button variant="outline" onClick={() => setTestingTool(null)}>
              <X className="h-4 w-4 mr-2" />
              Close
            </Button>
            <Button
              onClick={() => testingTool && handleTestTool(testingTool)}
              disabled={isTestLoading || !testingTool}
